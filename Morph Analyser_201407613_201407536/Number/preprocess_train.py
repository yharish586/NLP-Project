...
def gender_root_feature(f ,features,number,root):
    i=-1
    start=0
    for l in f:
        lines=l.rstrip('\n')
    ...
    if  not(lines.startswith('<Sentence ')or  lines.startswith('</Sentence>')or lines==(   '\t))')or lines==""):
        ...
        if word.startswith("<fs af="):
            ...
            if POS=='0' or POS=='':
                ...
                features[i][7]=POS
gender_root_feature(f,features,number,root)
print features
