...
def gender_root_feature(f ,features,category_test,root):
    i=-1
    start=0
    for l in f:
        lines=l.rstrip('\n')
        if  not(lines.startswith('<Sentence ')or  lines.startswith('</Sentence>')or lines==(   '\t))')or lines==""):
            ...
            for j in range(start+1,end):
                features[j][1]=category_test[j-1]
                features[j][2]=category_test[j+1]
                features[j][3]=root[j-1][8 :]
                features[j][4]=root[j+1][8 :]
            features[start][1]="none"
            features[start][2]=category_test[start+1]
            features[start][3]="none"
            features[start][4]=root[start+1][8 :]
            features[end][1]=category_test[end-1]
            features[end][2]="none"
...
features_test=gender_root_feature(f,features_test,category_test,root)
print features_test
